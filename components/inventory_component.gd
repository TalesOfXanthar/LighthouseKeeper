...
func add_item(item_name):
	...
	for item in inventory_container.get_children():
			if item.get_meta("name") == item_name:
				item.set_meta("amount", item.get_meta("amount") + 1)
				new_item = false
	if new_item:
		var item = TextureButton.new()
		#item.icon = null
		item.set_meta("name", item_name)
		item.set_meta("amount", 1)
...
func dictionary_to_inventory(item_dictionary : Dictionary):
	for item_name in item_dictionary.keys():
		for amount in range(item_dictionary[item_name]):
			add_item(item_name)

func inventory_to_dictionary():
	var items_dictionary := {}
	for item in items_group.get_buttons():
		items_dictionary.set(item.get_meta("name"), item.get_meta("amount"))
...
func pressed_item_to_menu(useless_var):
	...
	if selected_item_button != null:
		...
		item_profile.instate_item_profile(
			selected_item_button.get_meta("name"), 
			selected_item_button.get_meta("amount"), 
			item_position, 
			top_floor
			)
...
func item_button_action(item_name, tag_type, triggers_counteraction := true):
	...
	if triggers_counteraction && !top_floor:
		action_performer.perform_action()

func remove_item(item_name):
	var item_button : Node
	for item in inventory_container.get_children():
			if item.get_meta("name") == item_name:
				item_button = item
	if item_button.get_meta("amount") >= 2:
		item_button.set_meta("amount", item_button.get_meta("amount") - 1)
	else:
